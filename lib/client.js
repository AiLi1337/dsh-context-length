...
// (`remote.llm`, `remote.settings.mutate`, `configForms.describe()`),
// so the host stays the single fact source and
	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				".dcl-dropdown-pop{position:absolute;top:calc(100% + 6px);left:0;right:0;z-index:35;background:var(--dsw-alias-bg-module-platform);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:0 12px 32px -4px rgba(0,0,0,.22),0 4px 12px -2px rgba(0,0,0,.08);padding:6px;display:flex;flex-direction:column;gap:4px;max-height:260px;animation:dcl-dropdown-in .15s cubic-bezier(0.16,1,0.3,1)}",
				...
				".dcl-dropdown-options-list{display:flex;flex-direction:column;gap:2px;overflow-y:auto;max-height:175px;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;min-height:0;flex:1 1 auto;scrollbar-gutter:stable}",
				".dcl-dropdown-options-list::-webkit-scrollbar{width:6px}",
				".dcl-dropdown-options-list::-webkit-scrollbar-track{background:transparent}",
				".dcl-dropdown-options-list::-webkit-scrollbar-thumb{background:rgba(128,128,128,.4);border-radius:4px}",
				".dcl-dropdown-options-list::-webkit-scrollbar-thumb:hover{background:rgba(128,128,128,.7)}",
				...
				".dcl-effort-capsules{display:flex;flex-wrap:wrap;gap:8px;margin-top:4px}",
				".dcl-effort-pill{box-sizing:border-box;display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border-radius:10px;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);background:color-mix(in srgb,var(--dsw-alias-bg-layer-1) 85%,transparent);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:var(--dsw-alias-label-primary);font-size:12px;font-weight:500;line-height:18px;cursor:pointer;transition:all .16s ease;user-select:none}",
				".dcl-effort-pill:hover:not(:disabled){border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary);background:var(--dsw-alias-interactive-bg-hover-solid)}",
				".dcl-effort-pill.is-active{background:rgba(59,130,246,.12);border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary);font-weight:600;box-shadow:0 1px 4px rgba(59,130,246,.15)}",
				".dcl-effort-pill:disabled{opacity:.45;cursor:not-allowed}",
				".dcl-pill-check{font-size:11px;font-weight:bold;color:var(--dsw-alias-brand-primary)}",
				".dcl-pill-hint{font-size:11px;color:var(--dsw-alias-label-tertiary);margin-left:2px}",
					...
					open ? react_jsx_runtime.jsxs("div", {
						...
						onWheel: (event) => {
							if (selectListRef.current) {
								...
								selectListRef.current.scrollTop += event.deltaY * factor;
							}
						},
						children: [
		...
		function ContextLengthSection(props) {
			const { remote, schema, describe, t } = props;
			...
			/**
			 * 读取渠道目录与设置快照。载体（浏览器 ↔ 宿主 RPC 通道）可能瞬时不可用
			 * （页面加载/重连瞬间会抛 "Failed to fetch"），故失败后退避重试几次，
			 * 避免一进页面就停在错误态。
			 * @param attempts - 尝试次数；非数字（被当作事件参数传入）时用默认值。
			 */
			const load = react.useCallback(async (attempts) => {
				const limit = typeof attempts === "number" ? attempts : 3;
				setPhase("loading");
				setError("");
				for (let attempt = 1; attempt <= limit; attempt += 1) {
					try {
						...
						const list = providersResponse.value
			...
			const commitModels = async (targetProviderId, targetModelId, transform, extraOps = []) => {
				...
				const response = await remote.settings.mutate(ns.ns, ops, ns.revision);
				if (!response.ok) {
					throw new Error(response.error.code === "settings/conflict"
						? t("conflict")
										...
										children: [
											EFFORT_LEVELS.map((level) => {
												...
												return react_jsx_runtime.jsxs("button", {
													type: "button",
													className: `dcl-effort-pill ${active ? "is-active" : ""}`,
													...
													children: [
														react_jsx_runtime.jsx("span", {
															className: "dcl-pill-check",
															children: active ? "✓" : "+"
														}),
														react_jsx_runtime.jsx("span", {
															children: t("effort" + level[0].toUpperCase() + level.slice(1))
														}),
														level === "off" ? react_jsx_runtime.jsx("span", {
															className: "dcl-pill-hint",
															children: `(${t("effortOffHint")})`
														}) : null
													]
												}, level);
											})
		...
		// ── cordis plugin entry ──
		const inject = ["slots", "locale", "remote", "remote.llm", "remote.settings", "configForms", "settingsSchema"];
		...
		function apply(ctx) {
			...
			// DSH 0.1.2+ 的 RPC 从 connection.api 迁移到 ctx.remote（typert client remote）。
