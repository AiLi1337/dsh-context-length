	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				".dcl-dropdown-options-list::-webkit-scrollbar,.dcl-effort-options-list::-webkit-scrollbar{width:6px}",
				".dcl-dropdown-options-list::-webkit-scrollbar-track,.dcl-effort-options-list::-webkit-scrollbar-track{background:transparent}",
				".dcl-dropdown-options-list::-webkit-scrollbar-thumb,.dcl-effort-options-list::-webkit-scrollbar-thumb{background:rgba(128,128,128,.4);border-radius:4px}",
				".dcl-dropdown-options-list::-webkit-scrollbar-thumb:hover,.dcl-effort-options-list::-webkit-scrollbar-thumb:hover{background:rgba(128,128,128,.7)}",
				...
				".dcl-effort-pop{position:absolute;top:calc(100% + 6px);left:0;right:0;z-index:50;background:var(--dsw-alias-bg-module-platform);background:color-mix(in srgb,var(--dsw-alias-bg-module-platform) 82%,transparent);backdrop-filter:blur(18px) saturate(180%);-webkit-backdrop-filter:blur(18px) saturate(180%);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:0 12px 32px -4px rgba(0,0,0,.22),0 4px 12px -2px rgba(0,0,0,.08);padding:6px;display:flex;flex-direction:column;gap:4px;animation:dcl-dropdown-in .15s cubic-bezier(0.16,1,0.3,1)}",
				".dcl-effort-options-list{display:flex;flex-direction:column;gap:2px;overflow-y:auto;max-height:165px;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;min-height:0;flex:1 1 auto;scrollbar-gutter:stable}",
				...
				".dcl-effort-pop-hint{font-size:11px;color:var(--dsw-alias-label-tertiary);padding:6px 8px 2px;border-top:1px solid var(--dsw-alias-border-l1);margin-top:2px;flex-shrink:0}",
				".dcl-effort-backdrop{position:fixed;inset:0;z-index:40;background:transparent}",
		...
		function ContextLengthSection(props) {
			...
			const effortListRef = react.useRef(null);
									...
									effortSupport === "ok" && reasoningState.kind !== "disabled" ? react_jsx_runtime.jsxs("div", {
										...
										children: [
											...
											effortOpen ? react_jsx_runtime.jsxs("div", {
												...
												onWheel: (event) => {
													if (effortListRef.current) {
														event.preventDefault();
														const factor = event.deltaMode === 1 ? 16 : 1;
														effortListRef.current.scrollTop += event.deltaY * factor;
													}
												},
												children: [
													react_jsx_runtime.jsx("div", {
														className: "dcl-effort-options-list",
														ref: effortListRef,
