	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				".dcl-effort-pop{position:absolute;top:calc(100% + 6px);left:0;right:0;z-index:35;background:var(--dsw-alias-bg-module-platform);background:color-mix(in srgb,var(--dsw-alias-bg-module-platform) 82%,transparent);backdrop-filter:blur(18px) saturate(180%);-webkit-backdrop-filter:blur(18px) saturate(180%);border:1px solid var(--dsw-alias-border-l2);border-radius:12px;box-shadow:0 12px 32px -4px rgba(0,0,0,.22),0 4px 12px -2px rgba(0,0,0,.08);padding:6px;display:flex;flex-direction:column;gap:4px;animation:dcl-dropdown-in .15s cubic-bezier(0.16,1,0.3,1)}",
				".dcl-effort-options-list{display:flex;flex-direction:column;gap:2px;overflow-y:auto;max-height:190px;overscroll-behavior:contain;-webkit-overflow-scrolling:touch}",
